...
public class RedBlackTreeConverter {
    ...
    public static<Key extends Comparable<Key>> RBNode<Key> convert(TwoThreeNode<Key> twoThreeNode) {
        ...
        // if is3node => add keyRight, keyRight rightChild is the 3node rightChild / add keyLeft as leftChild of the rightChild (red link), keyLeft keeps both left and center children
        else if (twoThreeNode.is3node()) {
            RBNode<Key> blackNode = new RBNode<>(twoThreeNode.rightKey, twoThreeNode.rightValue, Color.Black, 1);
            //RBNode<Key> redBlackNode = new RBNode<>(twoThreeNode.rightKey, twoThreeNode.rightValue, Color.Black, 1); // (Key key, int value, Color color, int size)

            // keep rightChild under new normal node
            blackNode.rightChild = convert(twoThreeNode.rightChild);
            //redBlackNode.rightChild = new RBNode<>(twoThreeNode.rightChild.leftKey, twoThreeNode.rightChild.leftValue, Color.Black, 0);

            // lower leftKey on the left as a RED node
            RBNode<Key> redNode = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Red, 1);
            //redBlackNode.leftChild = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Red, 0);

            // children of leftKey are leftChild and CenterChild
            redNode.leftChild = convert(twoThreeNode.leftChild);
            //redBlackNode.leftChild.leftChild = new RBNode<>(twoThreeNode.leftChild.leftKey, twoThreeNode.leftChild.leftValue, Color.Black, 0);
            redNode.rightChild = convert(twoThreeNode.centerChild);
            //redBlackNode.leftChild.rightChild = new RBNode<>(twoThreeNode.centerChild.leftKey, twoThreeNode.centerChild.leftValue, Color.Black, 0);
            redNode.size += sizeEvenIfNull(redNode.leftChild) + sizeEvenIfNull(redNode.rightChild);

            blackNode.leftChild = redNode;
            blackNode.size += redNode.size + sizeEvenIfNull(blackNode.rightChild);

            return blackNode;
        }
        // if is2node => just add it plain and simple
        else { // if (twoThreeNode.is2node())
            RBNode<Key> redBlackNode = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Black, 1);
            redBlackNode.rightChild = convert(twoThreeNode.centerChild);
            //redBlackNode.rightChild = new RBNode<>(twoThreeNode.centerChild.leftKey, twoThreeNode.centerChild.leftValue, Color.Black, 0);
            redBlackNode.leftChild = convert(twoThreeNode.leftChild);
            //redBlackNode.leftChild = new RBNode<>(twoThreeNode.leftChild.leftKey, twoThreeNode.leftChild.leftValue, Color.Black, 0);
            redBlackNode.size += sizeEvenIfNull(redBlackNode.leftChild) + sizeEvenIfNull(redBlackNode.rightChild);
