...
public class SleighRanking {
    ...
    private static class BST {
        ...
        private static class Node {
            ...
            private int n_smaller_childrens = 0;
            private int count = 1;
        ...
        public int putAndRank(int key) {
            Rank rank = new Rank(); // rank is an object so that it is updated when passed in parameters
            root = putAndRank(root, key, rank); // update the tree
            return rank.rank;
        }

        private Node putAndRank(Node node, int key, Rank rank) {
            if (node == null) return new Node(key); // add the node
            if (key < node.key) { // go left
                node.left = putAndRank(node.left, key, rank);
                node.n_smaller_childrens++; // for the newly added node
            }
            else if (node.key < key) { // go right
                rank.rank += node.n_smaller_childrens + node.count; // rank = everything that is smaller (+ potential duplicates)
                node.right = putAndRank(node.right, key, rank);
            }
            else { // duplicate
                rank.rank += node.n_smaller_childrens; // rank = everything that is smaller
                node.count++; // record that there was a duplicate
            }
            return node;
        }
    }
    ...
    public static int[] computeRankings(int[] array) {
        // TODO: Did I already tell you how much I HATE trees ? I now declare it
        // [5, 4, 6, 4, 2, 3, 7]
        BST bst = new BST();
        for (int i = 0; i < array.length; i++) array[i] = bst.putAndRank(array[i]);
        // [0, 0, 2, 0, 0, 1, 6]
        return array;
    }
