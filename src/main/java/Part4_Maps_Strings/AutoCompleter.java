...
public class AutoCompleter {
     ...
     */
    private static int charToIndex(char c) { return ((int) c) - 97; }
     ...
     */
    private static char indexToChar(int i) { return (char) (i + 97); }
     ...
     */
    private void insert(String word) { this.root = insert(root, word, 0); }
     ...
     * you can look it <a href="https://en.wikipedia.org/wiki/Breadth-first_search">here</a>
     *
    ...
    public String complete(String prefix) {
        ...
        for (int i = 0; i < prefix.length(); i++) {
            startNode = startNode.next[charToIndex(prefix.charAt(i))];
            if (startNode == null) return null;
        }
        if (startNode.isKey) return prefix; // prefix is already a word
        ...
        while (!q.isEmpty()) {
            ...
            if (currentNode.isKey) return currentWord;
            for (int i = 0; i < currentNode.next.length; i++) {
                Node child = currentNode.next[i];
                if (child != null) {
                    q.add(child);
                    words.add(currentWord+indexToChar(i));
                }
            }
        }
        return null;
    }
    // Note : plusieurs queues c'est utile pour plusieurs trucs diff dans un BFS
}
