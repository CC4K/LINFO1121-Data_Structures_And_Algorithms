...
public class LibraryStore {
    ...
    public static class Book {
        ...
        public String toString() {
            return String.format("(%d %d)", isbn, year);
        }
    }

    int size; // number of keys in the hash table
    final double loadFactor; // the load factor that triggers resizing
    LinkedList<Book>[] buckets;

    /**
     * Constructs a library store with the specified initial capacity with a default load factor of 0.75.
    ...
    public LibraryStore(int M, double loadFactor) {
        if (loadFactor <= 0) throw new IllegalArgumentException("Load factor must be positive");
     ...
     * <p>
     * It should resize (double the capacity of the internal array) when the load factor is exceeded.
    ...
    public void add(int isbn, int year) {
        ...
        // TODO: juste call resize() when (number of keys / number of buckets) > load factor
        if (((double) getSize() / getCapacity()) > loadFactor) resize();
    }
     ...
     */
    private void resize() {
        // TODO: should double the buckets table size and re-hash everything
        LibraryStore bigger_store = new LibraryStore(getCapacity()*2, loadFactor);
        for (LinkedList<Book> bucket : buckets) {
            for (Book book : bucket) {
                bigger_store.add(book.isbn, book.year);
            }
        }
        buckets = bigger_store.buckets;
    }
    ...
    public void delete(int isbn) {
        // TODO
        LinkedList<Book> bucket = buckets[hash(isbn)]; // find correct bucket
        bucket.removeIf((book) -> book.isbn == isbn);
        size--;
    }
