...
public class ArrayBSTDelete<Key extends Comparable<Key>, Value> {
    ...
    public ArrayList<Boolean> removed;
    ...
    private void addNode(Key key, Value val) {
        ...
        removed.add(false); // "still present" state
    }
        ...
        if (!values.isEmpty()) {
            ...
            do {
                ...
                if (cmp == 0) {
                    ...
                    values.set(i,val);
                    // un-remove the removed node (if it was present)
                    if (removed.get(i)) { // TODO: modified
                        removed.set(i, false);
                        return true;
                    }
                    return false;
...
//        System.out.println(idxRightNode);
        ...
        if (removed.get(i)) return null; // TODO: modified
        return values.get(i);
    }
...
//        System.out.println(idxRightNode);
        ...
        if (idx == NONE) return false; // not found
        if (!removed.get(idx)) { // found => remove
            removed.set(idx, true);
            return true;
        }
        else return false; // found but was already removed (nothing to do)
    }
