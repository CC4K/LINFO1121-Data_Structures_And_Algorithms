 ...
 * In order to save humanity, we ask you to design an optimal escape plan to a habitable galaxy using only functional by-passes.
...
public class GalaxyPath {
     ...
     *            iff n1 = source, nk = destination and for all (ni, ni+1) in P, graph[ni][ni+1] >= i+1
     *         -1 if no path exists
     *
     */
    public static int findPath(int[][] graph, int source, Set<Integer> destinations) {
        ...
        while (!frontier.isEmpty()) {
            int galaxyId = frontier.poll();
            // check if a destination was reached
            if (destinations.contains(galaxyId)) {
                //System.out.println(Arrays.toString(distTo));
                return distTo[galaxyId];
            }
            // check neighbours
            for (int neighbourId = 0; neighbourId < graph.length; neighbourId++) {
                int bypass = graph[galaxyId][neighbourId];
                if (bypass != 0) {
                    if (!marked[neighbourId] && (bypass > distTo[galaxyId])) {
                        marked[neighbourId] = true;
                        distTo[neighbourId] = distTo[galaxyId] + 1;
