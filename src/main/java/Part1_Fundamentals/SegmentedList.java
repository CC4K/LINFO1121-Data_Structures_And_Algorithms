...
class SegmentedListImpl<T> implements SegmentedList<T> {
    ...
    List<List<T>> segList = new ArrayList<>();
    ...
    public void removeSegment(int index) {
        ...
        List<T> segToRemove = segList.get(index);
        fullList.removeAll(segToRemove);
        segList.remove(index);
        size -= segToRemove.size();
        System.out.println(fullList);
        System.out.println(segList);
    }
    ...
    public T get(int globalIndex) {
         return fullList.get(globalIndex);
    }
