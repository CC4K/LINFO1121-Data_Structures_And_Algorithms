...
public class Huffman {
    ...
    public static HuffmanNode buildTrie(int[] freq) {
        ...
        // t 110
        // m 111
        PriorityQueue<HuffmanNode> pq = new PriorityQueue<>(); // minPQ sorted by frequency
        for (char c = 0; c < freq.length; c++) pq.add(new HuffmanNode(c, freq[c], null, null));
        //for (HuffmanNode huffmanNode : pq) System.out.println(huffmanNode);
        while (pq.size() > 1) {
            HuffmanNode leftChild = pq.poll();
            HuffmanNode rightChild = pq.poll();
            HuffmanNode merged = new HuffmanNode('_', leftChild.getFrequency()+rightChild.getFrequency(), leftChild, rightChild);
            pq.add(merged);
        }
        return pq.poll();
    }
...
class HuffmanNode implements Comparable<HuffmanNode> {
    ...
    public String toString() {
        return String.format("%d %d %s %s", ch, freq, left, right);
    }
